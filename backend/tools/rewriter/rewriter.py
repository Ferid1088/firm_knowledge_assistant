...
from backend.tools.rewriter import protected
...
def _load_prompt_template(lang: str) -> str:
    ...
    p = Path("backend/prompts") / f"rewrite_{ld.prompt_key}.txt"
    if p.exists():
        return p.read_text(encoding="utf-8")
    fallback = Path("backend/prompts") / "rewrite_de.txt"
...
def _fewshots_path(lang: str) -> str:
    from backend.services.language import registry
    ld = registry.get(lang)
    p = Path("backend/rewrite_data") / f"rewrite_fewshots_{ld.prompt_key}.yaml"
    if p.exists():
        return str(p)
    fallback = Path("backend/rewrite_data") / "rewrite_fewshots_de.yaml"
    return str(fallback)
