/**
 * Transparent proxy for /api/auth/* → FastAPI backend.
 * Forwards cookies in both directions so the session cookie flows correctly.
 */
...
async function proxy(req: NextRequest, params: { path: string[] }) {
  ...
  const url = `${BACKEND_ORIGIN}/api/auth/${path}`;
